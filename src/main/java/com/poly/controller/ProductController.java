...
public class ProductController {
	...
	public String getDsSanPham(Model model)
	{
		...
		List<SanPham> ds = spDAO.findAll();
		List<DanhMuc> dm3 = dmDAO.findAllByItem();
		List<DanhMuc> dm = dmDAO.findAllByModel();
		List<DanhMuc> dm2 = dmDAO.findAllByType();
		model.addAttribute("dssanpham", ds);
		model.addAttribute("DM_Hang", dm);
		model.addAttribute("DM_Loai", dm2);
		model.addAttribute("DM_PK", dm3);
	...
	public String locSPTheoDS(@PathVariable("id") Integer id_dm, Model model)
	{
		...
		List<SanPham> dssanpham = spDAO.findAllByIdDM(id_dm);
...
//				}
	...
	public String search(@PathVariable("tensp") String keywords, Model model, @RequestParam("page") Optional<Integer> page) {
		hienThiGioHang(model);
		getDsSanPham(model);
	...
	public String searchGia(Model model, @RequestParam("price") double price,@RequestParam("page") Optional<Integer> page ) {
		hienThiGioHang(model);
		getDsSanPham(model);
		Pageable pageable = PageRequest.of(page.orElse(0), 5);
		Page<SanPham> pages = spDAO.findAll(pageable);
...
//	}
//	@ModelAttribute("DM_PK")
//	public List<DanhMuc> getListPhKien()
//	{
//		return dmDAO.findAllByType();
//	}
